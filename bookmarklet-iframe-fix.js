javascript:(function(){
  if(window.fpMS){alert('Already active!');return;}
  ...
  b.textContent='🎯 Active - Click fields in iframe (ESC to stop)';
  document.body.appendChild(b);
  
  var upd=function(){
    document.getElementById('fpC').textContent=w.f.length;
    var s=document.getElementById('fpS');
    var j=document.getElementById('fpJ');
    s.disabled=w.f.length===0;
    j.disabled=w.f.length===0;
    s.style.opacity=w.f.length?'1':'0.5';
    ...
    if(!w.f.length){
      ...
      l.innerHTML=w.f.map(function(f,i){
        return'<div style="background:#f8f9fa;padding:12px;border-radius:8px;margin-bottom:12px;border-left:4px solid #667eea"><div style="display:flex;justify-content:space-between;margin-bottom:8px"><div style="font-weight:600;font-size:14px">'+eh(f.label||f.id)+'</div><button onclick="window.fpMS.rm('+i+')" style="background:#e74c3c;color:white;border:none;padding:4px 10px;border-radius:4px;cursor:pointer;font-size:11px">Remove</button></div><div style="font-size:12px;color:#7f8c8d">ID: '+eh(f.id)+' | Type: '+eh(f.fieldType)+'</div></div>';
      }).join('');
    }
  ...
  w.rm=function(i){w.f.splice(i,1);upd();};
  
  var act=function(){if(w.a)return;w.a=1;upd();};
  var deact=function(){if(!w.a)return;w.a=0;upd();};
  ...
  var clk=function(e){
    if(!w.a||e.target.closest('#fpP')||e.target.closest('#fpB'))return;
    ...
    if(al){
      var le=f.ownerDocument.getElementById(al);
      if(le)lbl=le.textContent.trim();
    }else{
      var lf=f.ownerDocument.querySelector('label[for="'+f.id+'"]');
      if(lf)lbl=lf.textContent.trim();
    }
    lbl=lbl.replace(/[*:]/g,'').replace(/\s*(Required|Mandatory)\s*/gi,'').trim();
    
    var ft='text';
    if(f.tagName==='SELECT')ft='combobox';
    else if(f.tagName==='TEXTAREA')ft='text';
    else if(f.type)ft=f.type;
  ...
  var esc=function(e){if(e.key==='Escape')deact();};
  ...
  for(var i=0;i<frames.length;i++){
    try{
      frames[i].contentDocument.addEventListener('click',clk,true);
      frames[i].contentDocument.addEventListener('keydown',esc);
    }catch(e){
  ...
  document.getElementById('fpS').onclick=function(){
    if(!w.f.length){alert('No fields!');return;}
    localStorage.setItem('jiraLinkGen_pendingFieldImport',JSON.stringify({timestamp:Date.now(),fields:w.f}));
    alert('✅ Saved '+w.f.length+' fields!\n\nGo back to Link Generator tab.');
    w.f=[];
    upd();
    deact();
  };
